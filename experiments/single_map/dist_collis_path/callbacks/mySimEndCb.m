function fits = mySimEndCb(state, my_state, log_folder)
    ...
    fits = my_state.fitness();
    
    [best_fit, dists, collis, path_lens] = my_state.bestFit();
    ...
    logger.debug('Distance | Collision | Path len');
    logger.debug(sprintf('\n%f\t|\t%d\t|\t%f', [dists, collis, path_lens]'));
    
    % some of my internal state properties need to be re-set
    my_state.reset();
    
    % increment my internal generation counter
